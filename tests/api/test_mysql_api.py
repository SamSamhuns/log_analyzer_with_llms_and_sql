...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            (
                "SELECT * FROM data WHERE id = %s AND name = %s AND salary > %s AND birth_date = %s",
                (5, "John", 50000.75, "1990-05-21"),
            ),
        ),
    ],
)
...
def test_insert_sql(test_mysql_connec: Connection, gen_mock_anomaly_det_log_data: Callable):
    ...
    resp = insert_data_into_sql(
        ...
        gen_mock_anomaly_det_log_data(MYSQL_TEST_ID),
    )

    assert resp == {"status": "success", "message": "record inserted into mysql db"}
...
def test_select_sql(test_mysql_connec: Connection, gen_mock_anomaly_det_log_data: Callable):
    ...
    resp = select_data_from_sql_with_id(test_mysql_connec, MYSQL_TEST_ANOMALY_DET_LOG_TABLE, MYSQL_TEST_ID)
    assert resp == {
        "status": "success",
...
def test_select_all_sql(test_mysql_connec, gen_mock_anomaly_det_log_data):
    """Selects test data from MySQL database."""
    resp = select_all_data_from_sql(test_mysql_connec, MYSQL_TEST_ANOMALY_DET_LOG_TABLE)
    assert resp == {
        "status": "success",
...
def test_delete_mysql(test_mysql_connec: Connection):
    ...
    resp = delete_data_from_sql_with_id(test_mysql_connec, MYSQL_TEST_ANOMALY_DET_LOG_TABLE, MYSQL_TEST_ID)
    assert resp == {"status": "success", "message": "record deleted from mysql db"}
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"ID": MYSQL_TEST_ID - 1},
            True,
        ),  # ID - 1 was bulk inserted & not deleted, must be True
    ],
)
...
def test_run_select_sql_script_after_update(test_mysql_connec: Connection):
    ...
    resp1 = select_data_from_sql_with_id(test_mysql_connec, MYSQL_TEST_ANOMALY_DET_LOG_TABLE, MYSQL_TEST_ID - 1)
