    ...
    try:
        for file in files:
            ...
            if entries_exist(
                mysql_conn,
                MYSQL_LOG_ID_TB_NAME,
                {"file_md5": fmd5, "logfile_type": logfile_type},
            ):
...
async def file_upsert(
    ...
    try:
        for file in files:
            ...
            f_ext = osp.splitext(f_name)[-1].lower()
            if f_ext == ".pdf":
            ...
            elif embedding_model == EmbeddingModel.OPENAI_TEXT_EMBEDDING_MODEL:
                emb = OpenAIEmbeddings(model=embedding_model.value)

            Chroma.from_documents(documents=splits, embedding=emb, persist_directory=VECTOR_STORE_DIR)
